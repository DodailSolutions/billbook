...
import { updatePurchaseOrder, getPurchaseOrder } from '@/lib/po-actions'
...
export default function EditPurchaseOrderPage() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            getVendorsList(),
            getPurchaseOrder(id)
        ]).then(([invData, vendorData, poData]) => {
            ...
            if (poData) {
                ...
                setVendorName(poData.vendor_name)
                setVendorEmail(poData.vendor_email || '')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (result.success) {
            ...
            setError(result.error || 'Failed to update Purchase Order.')
            setLoading(false)
        }
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex items-center justify-end gap-3">
                    ...
                    <Button type="submit" disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 text-white min-h-[44px]">
                        {loading ? 'Saving PO...' : 'Save Draft Changes'}
                    </Button>
                </div>
            </form>
        </div>
