...
import { getPurchaseOrder } from '@/lib/po-actions'
...
export async function GET(
    ...
    try {
        ...
        console.log('PDF Route - Purchase Order ID:', id, 'Mode:', mode)
        ...
        const html = await generatePurchaseOrderPDF(po)
...
<head>
    <meta charset="UTF-8">
    <title>PurchaseOrder-${po.po_number}</title>
    <style>
        @media print {
            @page {
                margin: 1cm 1.5cm;
...
<body>
    ...
    `
        ...
        // If mode is 'html', return just the purchase order HTML without wrapper
        if (mode === 'html') {
        ...
        return new NextResponse(pdfHtml, {
            headers: {
                'Content-Type': 'text/html; charset=utf-8',
                'Content-Disposition': `inline; filename="PurchaseOrder-${po.po_number}.html"`,
                'Cache-Control': 'no-cache, no-store, must-revalidate',
            },
        })
    } catch (error) {
        console.error('Error in PO PDF route:', error)
